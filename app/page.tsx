...
import { ArrowRight, Sparkles, Wand2, Code2, Eye, ShieldCheck, Zap } from 'lucide-react';
...
const features = [
  ...
  { icon: Zap, title: 'Streaming AI', desc: 'Get responses token-by-token with lightning-fast interaction loops.' },
  { icon: ShieldCheck, title: 'Auth + Billing', desc: 'Secure builders with Clerk auth and Stripe subscription flows.' }
];

const testimonials = [
  { name: 'Aria K.', role: 'Founder', quote: 'The fastest way I have ever gone from idea to polished SaaS landing page.' },
  { name: 'Dylan R.', role: 'Agency Lead', quote: 'It feels like having a senior frontend engineer and designer on demand.' },
  { name: 'Mina S.', role: 'Solo Dev', quote: 'The chat + editor + preview workflow is exactly what I needed.' }
];
    ...
    <main className="min-h-screen bg-hero-gradient">
      ...
      <section className="mx-auto flex max-w-7xl flex-col items-center px-6 pb-20 pt-8 text-center">
        <motion.h1
          ...
          className="max-w-4xl bg-gradient-to-r from-white via-sky-200 to-violet-300 bg-clip-text text-5xl font-semibold leading-tight text-transparent sm:text-6xl"
        >
      ...
      <section className="mx-auto max-w-7xl px-6 pb-12">
        <motion.div initial={{ opacity: 0, y: 15 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="glass rounded-3xl p-8">
          <h2 className="text-center text-2xl font-semibold">Product demo workflow</h2>
          <div className="mt-6 grid gap-4 md:grid-cols-3">
            {['Describe your website', 'AI generates & edits code', 'Preview and export'].map((step, idx) => (
              <div key={step} className="rounded-2xl border border-white/15 p-4">
                <div className="mb-2 text-xs text-accent2">STEP {idx + 1}</div>
        ...
        <div className="glass rounded-3xl p-8 text-center">
          ...
          <div className="mt-6 grid gap-4 md:grid-cols-2">
            ...
            <div className="rounded-2xl border border-accent/50 bg-accent/10 p-6 text-left shadow-glow">
              ...
              <p className="mt-3 text-sm text-white/70">Unlimited projects, faster models, priority generations, and full export workflows.</p>
            </div>
          </div>
        </div>
      </section>
